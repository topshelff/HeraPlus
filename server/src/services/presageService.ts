...
/** Minimum frames required for Presage Engine to extract vitals (e.g. ~3+ seconds at 5 fps). */
const PRESAGE_MIN_FRAMES = 15
  ...
  async stopSession(sessionId: string): Promise<BiometricSummary> {
    ...
    if (usePresageVideoApi && session.storedFrames && session.storedFrames.length > 0) {
      ...
      if (frameCount < PRESAGE_MIN_FRAMES) {
        console.warn(
          `Presage Video API skipped: only ${frameCount} frames (need at least ${PRESAGE_MIN_FRAMES}). Use longer scan or fallback.`
        )
        try {
          ...
          console.warn('Fallback summary failed:', fallbackErr)
          activeSessions.delete(sessionId)
          return {
            avgBpm: 0,
            avgHrv: 0,
