...
export default function TriageExplanation({ diagnosis }: TriageExplanationProps) {
  const level = diagnosis?.urgencyLevel?.toString().toUpperCase()
  const style =
    level && level in urgencyStyles
      ? urgencyStyles[level as UrgencyLevel]
      : urgencyStyles.MODERATE
  const redFlags = Array.isArray(diagnosis?.redFlags) ? diagnosis.redFlags : []
  const differentialConsiderations = Array.isArray(diagnosis?.differentialConsiderations)
      ...
      {differentialConsiderations.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {differentialConsiderations.map((condition, i) => (
              <span
                key={i}
